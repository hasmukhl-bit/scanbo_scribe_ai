...
const PageRoot = styled("main")(({ theme }) => ({
  "--login-gutter": "clamp(12px, 2.2vw, 28px)",
  minHeight: "100dvh",
  display: "grid",
  placeItems: "center",
  padding: "var(--login-gutter)",
  background: "#eceef1",
  boxSizing: "border-box"
}));

const Frame = styled(Box)(({ theme }) => ({
  width: "min(1200px, calc(100vw - (var(--login-gutter) * 2)))",
  height: "min(860px, calc(100dvh - (var(--login-gutter) * 2)))",
  borderRadius: 24,
  overflow: "hidden",
...
const Right = styled(Box)(({ theme }) => ({
  display: "none",
  background: `linear-gradient(180deg, ${alpha(theme.palette.primary.main, 0.22)} 0%, ${alpha(
...
const WorkflowShell = styled(Box)(({ theme }) => ({
  ...
  boxShadow: "0 10px 24px rgba(17, 50, 82, 0.08)",
  display: "flex",
        ...
        <Left>
          ...
          <Stack spacing={1.4} sx={{ pt: 0.6 }}>
            ...
            <Typography variant="h6" color="#334155" textAlign="center" fontWeight={500} sx={{ mt: 0.4 }}>
              Don&apos;t have an Account? <AuthTextLink href="/signup">Sign up</AuthTextLink>
            </Typography>
          </Stack>
        </Left>
